...
DATA_PATH2 = r'C:\Users\Rahul Chawla\Desktop\Universiy\NVD\NVD-aloy\PresentationVids'
...
for action in actions:
    ...
    for vid in os.listdir(action_dir):
        ...
        if len(sequence) == 250:
            ...
            while frame_num < 275:
                ...
                if(action == "fidgety_movements"):
                    true_val = "Normal Movements"
                    prediction = "Normal Movements"
                else:
                    true_val = "Abnormal Movements"
                    prediction = "Abnormal Movements"
                ...
                # cv2.imshow("image", img)
                # cv2.waitKey(1)
                cv2.putText(img, "AI prediction: ", (50, 250), cv2.FONT_HERSHEY_PLAIN, 2.5, (0, 0, 0),
                            3)
                cv2.putText(img, prediction, (50, 275), cv2.FONT_HERSHEY_PLAIN, 2.5, (255, 0, 0),
                            3)

                cv2.putText(img, "True Category:", (50, 350), cv2.FONT_HERSHEY_PLAIN, 2.5, (0, 0, 0), 3)
                cv2.putText(img, true_val, (50, 375), cv2.FONT_HERSHEY_PLAIN, 2.5, (255, 0, 0),
                            3)
                cv2.putText(img, "Confidence: ", (50, 450), cv2.FONT_HERSHEY_PLAIN, 2.5,
                            (0, 0, 0), 3)
                cv2.putText(img, str(np.round(np.max(res), 3)), (50, 475), cv2.FONT_HERSHEY_PLAIN, 2.5,
                            (255, 0, 0), 3)
                cv2.imshow("image", img)
                cv2.waitKey(1)
        ...
        i = i+1
        out = cv2.VideoWriter('overlayed' + str(vid) + '.mp4', cv2.VideoWriter_fourcc(*'DIVX'), 30, size)
