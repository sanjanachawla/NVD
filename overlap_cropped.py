...
number_of_clips = 2*math.floor(((end_hour*3600+ end_min*60 + end_sec) - (start_hour*3600 + start_min*60 + start_sec))/(length_clip_hour*3600 + length_clip_min*60 + length_clip_sec))
...
for i in range (number_of_clips):
    new = file.subclip(t_start = (start_hour + i*length_clip_hour/2, start_min + i*length_clip_min/2, start_sec + i*length_clip_sec/2), t_end = (start_hour + (i*length_clip_hour+2*length_clip_hour)/2, start_min + (i*length_clip_min+2*length_clip_min)/2, start_sec + (i*length_clip_sec+2*length_clip_sec)/2))
    new.write_videofile(r'C:\Users\aloys\Documents\Engineering\NVD\cerebral palsy\dataset\overlapped_5_sec_clips\abnormal_fidgety/' + str(i) + '.mp4')